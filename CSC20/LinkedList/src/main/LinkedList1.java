...
public class LinkedList1 {
    ...
    private Node head = null, current = null, tmp = null;
    ...
    public LinkedList1() {
        ...
        head = current = tmp = null;
    }
    // Array of elements
    public LinkedList1(Object[] list) {
    ...
    public boolean moveBackward() {
        if(current != head) {
            Node prev = head;
            while(prev.getLink() != current && prev.getLink() != null)
                prev = prev.getLink();
            if(prev != null) {
                current = prev;
                return true;
    ...
    /** Move the tmp position backward one position */
    private boolean back() {
        ...
        while(tmp.getLink() != place && tmp.getLink() != null) {
    ...
    public boolean removeLast() {
        if(isEmpty()) return false;
        if(tmp == head) return removeFirst();
